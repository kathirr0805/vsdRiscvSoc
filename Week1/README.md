...
Steps:
Download the Toolchain Used wget to download the xpack-riscv-none-elf-gcc archive.
Bash
...
Unpack the Archive Extracted the contents of the downloaded tarball. This created a folder named xpack-riscv-none-elf-gcc-14.2.0-3. 
Bash

tar -xvzf xpack-riscv-none-elf-gcc-14.2.0-3-linux-x64.tar.gz

Add the Toolchain to PATH (Temporarily) Added the toolchain binaries to the current session's PATH. 
Bash

export PATH=$PWD/xpack-riscv-none-elf-gcc-14.2.0-3/bin:$PATH

Check if Binaries Work Verified the functionality of gcc, objdump, and gdb by checking their versions. 
Bash
...
// hello.c
int main() {
    volatile char *uart = (char *)0x10000000; // Memory-mapped UART address [cite: 489]
    const char *msg = "Hello, World!\n";

    while (*msg) {
        *uart = *msg++; // Write each character to UART [cite: 490]
    }
    return 0;
}

// Minimal_start (no C runtime)
void _start() {
    main();
    while (1); // Trap CPU after main returns [cite: 491]
}
...
ENTRY(_start)
SECTIONS {
    . = 0x80000000; /* Code starts here */ [cite: 492]
    .text : {
        *(.text*)
    }
    .data : {
...
-march=rv32imc -mabi=ilp32 \
-nostartfiles -nostdlib -T linker.ld
This produced hello.s. 

Step 2: Look at the Assembly (hello.s)
An example snippet of the main function assembly is shown below, illustrating stack allocation and register saving/restoring. 

Code snippet

main:
    addi    sp,sp,-32   # Allocate 32 bytes stack [cite: 500]
    sw      ra,28(sp)   # Save return address [cite: 500]
    sw      s0,24(sp)   # Save frame pointer [cite: 500]
    addi    s0,sp,32    # Set up frame pointer (s0=old sp) [cite: 500]
    # ... function body ...
    lw      ra,28(sp)   # Restore return address [cite: 500]
    lw      s0,24(sp)   # Restore frame pointer [cite: 500]
    addi    sp,sp,32    # Deallocate stack [cite: 500]
    ret                 # Return [cite: 500]
Step 3: Explain Prologue and Epilogue
Prologue (Function Entry) 
The prologue allocates stack space, saves the return address (ra), and the caller's frame pointer (s0). It then sets up the new frame pointer. This ensures safe function calls and isolation of local variables. 


Code snippet

addi    sp,sp,-32   # Allocate stack space [cite: 502]
sw      ra,28(sp)   # Save return address [cite: 502]
sw      s0,24(sp)   # Save frame pointer (s0) [cite: 502]
addi    s0,sp,32    # Set new frame pointer [cite: 504]
Epilogue (Function Exit) 
The epilogue restores the saved ra and s0 registers, frees the allocated stack space, and returns control to the caller. 

Code snippet

lw      ra,28(sp)   # Restore return address [cite: 504]
lw      s0,24(sp)   # Restore old frame pointer [cite: 504]
addi    sp,sp,32    # Free stack space [cite: 504]
ret                 # Return to caller [cite: 504]
Output:
...
This section describes how to convert the ELF file into a raw binary or hex format and how to disassemble it using objdump, explaining the columns of the disassembly output.
...
Used objdump to disassemble the ELF. 
...
Caller-saved: a0–a7, t0–t6  These registers may be overwritten by a called function. If a caller wants to preserve them, it must save them before the call. 
...
(gdb) target sim
Connected to the simulator. [cite: 522]
(gdb) load
Loading section ., size 0x... lma 0x80000000
Start address 0x80000000 [cite: 522]
(gdb) break main
Breakpoint 1 at 0x80000000: file hello2.c, line 2. [cite: 522]
(gdb) run
Starting program: /home/vboxuser/RISC/Projects/Week1/hello/hello2.elf
Breakpoint 1, main () at hello2.c:2
2 volatile int x = 42; [cite: 522]
(gdb) step
3 x = x + 1; [cite: 523]
(gdb) stepi
(gdb) info registers
...
riscv-none-elf-readelf -h hello2.elf
Confirmation was made for Machine: RISC-V, Class: ELF32, and Entry point address: 0x80000000. 
Checking program headers and sections: 

Bash

riscv-none-elf-readelf -l hello2.elf
Ensured .text and .rodata are in FLASH with R E flags, and .data and .bss are in RAM with R W flags. 

3. Run with QEMU:
The ELF was run in QEMU's virt machine. 

Bash

qemu-system-riscv32 -nographic -machine virt -bios none -kernel ~/RISC/Projects/Week1/uart/hello2.elf
-nographic: Sends UART output to the terminal. 
-machine virt: Uses UART at 0x10000000. 
-bios none: Bare-metal, no BIOS. 
-kernel: Loads the ELF. 
Expected Output:
Printing from UART
Value of x: 43
Output:
...
Ensured no errors and checked ELFs: 

Bash

riscv-none-elf-readelf -h hello2_O0.elf
riscv-none-elf-readelf -h hello2_O2.elf
Run the ELFs to Confirm Functionality:
Both ELFs were tested in QEMU to ensure expected UART output. 
...
Expected output for both: Value of x: 43 
...
qemu-system-riscv32 -nographic -machine virt -bios none -kernel ~/RISC/Projects/Week1/uart/hello2.elf
Expected output includes "Value of x: 43" and "Cycles taken: &lt;some number>". 
...
Expected Output: GPIO Toggled .......... (dots continue) 
Test with Spike: 
Bash

spike --isa=rv32im ~/RISC/Projects/Week1/uart/gpio_toggle.elf
Expected: A, GPIO Toggled, B, dots. 
Bare-Metal C Snippet to Toggle GPIO:
To prevent the compiler from optimizing away the store operations to a memory-mapped register, the volatile keyword is crucial. It tells the compiler not to optimize access to this memory location.

C

// gpio_toggle.c
volatile unsigned int *GPIO_REGISTER = (volatile unsigned int *)0x10012000; // GPIO address [cite: 330]
...
Using volatile ensures that every read and write to GPIO_REGISTER is performed and not optimized away by the compiler, which is critical for interacting with hardware registers.

Output:
(Please add your screenshot of the GPIO toggling output from QEMU/Spike here)
...
SECTIONS {
    .text 0x00000000 : { /* .text at address 0x00000000 */ [cite: 341, 342]
        *(.text*)
    }

    .data 0x10000000 : { /* .data at address 0x10000000 */ [cite: 341, 342]
        *(.data*)
    }
...
Compile Command:
When compiling, ensure to link with this custom linker script. 
...
Ensure _start.s contains the _start label and sets up the stack and jumps to main. 
...
crt0.S is the very first assembly file that executes after a RISC-V processor resets. It sets up the execution environment before the main() function is called, especially crucial when a full C library like newlib or glibc is not used. 
...
Expected Output: S A Timer enabled .MTIP .MTIP ... (MTIP every ~1s, dots continue) 
Output:
...
Atomicity: Atomic operations ensure that a sequence of memory accesses (e.g., read-modify-write) is performed as a single, uninterruptible unit, preventing race conditions when multiple hardware threads (harts) access shared memory. 
...
sc.w rd, rs2, (rs1) (Store-Conditional Word): Attempts to store rs2 to rs1. Succeeds (writes 0 to rd) only if the reservation is still valid. Fails (writes non-zero to rd) if another hart modified the address. 
...
Expected Output demonstrates synchronized access to a shared counter by two pseudo-threads. 
Example Output:
...
Example _write Implementation (Conceptual):
C
...
// You would also need a _read and _sbrk for a fully functional printf/scanf
// and heap if using malloc.
By implementing _write this way, any call to printf will ultimately direct its output characters to the specified memory-mapped UART register.
